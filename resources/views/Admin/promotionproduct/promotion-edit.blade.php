                ...
                <div id="edit">
                    <form action="{{url('updatepromotionproduct/'.$promotion->id_promotion.'')}}"  method="post" id="save">            
                        ...
                        <div class="row" >
                            ...
                            <div class="col-4">
                                <select class="form-control type" name="type" required>
                                    ...
                                    <option value="2" {{$promotion->type == 2 ? 'selected' : ''}} disabled>ซื้อครบ X บาท รับพรีเมียม</option>
                                    <option value="3" {{$promotion->type == 3 ? 'selected' : ''}} disabled>ซื้อครบ X บาท ลดเพิ่ม Y บาท</option>
                                    <option value="4" {{$promotion->type == 4 ? 'selected' : ''}} disabled>ซื้อครบ X บาท ลดเพิ่ม Y % </option>
                                    <option value="5" {{$promotion->type == 5 ? 'selected' : ''}} disabled>ลด X % ทั้งร้าน</option>
                                </select>
                             </div>
                        </div>
                        <br>
                        ...
                        <div id="type1" >
                            ...
                            <br>
                            @foreach ($all as $_all)
                            <div class="row mt-2" id="item{{$_all->id_promotion_type}}">
                                <div class="col-2 mt-2">
                                    สินค้าชิ้นที่ 1 
                                </div>
                                <div class="col-3">
                                    <select class="form-control product" name="get_product_1[{{$_all->id_promotion_type}}]" required>
                                 ...
                                 <div class="col-1">
                                   <input type="number" class="form-control" name="count_1[{{$_all->id_promotion_type}}]" value="{{$_all->count_1}}">
                                 </div>
                                 <div class="col-1  mt-2">
                                    สินค้าชิ้นที่ 2
                                </div>
                                <div class="col-3">
                                    <select class="form-control product" name="get_product_2[{{$_all->id_promotion_type}}]" required>
                                        <option>-เลือกสินค้า-</option>
                                        @foreach ($products as $item)
                                ...
                                <div class="col-1">
                                    <input type="number" class="form-control" name="count_2[{{$_all->id_promotion_type}}]"  value="{{$_all->count_2}}">
                                </div>
                                <div class="col-sm">
                                    <button  type="button" class="btn btn-danger" onclick="deleteitem({{$_all->id_promotion_type}})" style="position: absolute; top: 0px;"><i class="fas fa-trash"></i></button>
                                </div>
                            </div>
                            @endforeach
...
<?php $count = DB::table('promotion_type1')->max('id_promotion_type'); ?>
@endsection
